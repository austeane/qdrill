...
describe('Sorting Controls', () => {
  ...
  const getItemsText = (selector) => {
    const texts = [];
    return cy.get(selector).each(($el) => texts.push($el.text().trim().toLowerCase()))
      .then(() => texts);
  };
  ...
  const testSorting = (url, apiAlias, itemSelector, nameSelector, dateSelector) => {
    ...
    if (dateSelector) {
        ...
        cy.get(`${itemSelector}:first ${dateSelector}`).then($el => firstItemDateAsc = $el.text());
  ...
  it('should sort Drills correctly', () => {
    ...
    testSorting(drillsUrl, 'getDrills', '[data-testid="drill-card"]', '[data-testid="drill-card-name"]', '[data-testid="drill-card-date"]');
  });

  it('should sort Formations correctly', () => {
     // Assuming data-testid for name and date within the card
    testSorting(formationsUrl, 'getFormations', '[data-testid="formation-card"]', '[data-testid="formation-card-name"]', '[data-testid="formation-card-date"]');
  });
}); 