...
<div class="section-header flex items-center gap-4 mb-4">
	...
	/>
	<button class="text-blue-500 hover:text-blue-700 text-sm" on:click={openDrillSearch}>
		Add Drill
	</button>
	<button class="text-blue-500 hover:text-blue-700 text-sm" on:click={openTimelineSelector}>
		Create Parallel Block
	</button>
	<button class="text-red-500 hover:text-red-700" on:click={() => onRemove(section.id)}>
		Remove Section
	</button>
