...
describe('BaseEntityService', () => {
  ...
  describe('search', () => {
    it('should search entities by terms', async () => {
      ...
      expect(mockDb.query).toHaveBeenCalledTimes(2);
      expect(mockDb.query.mock.calls[0][0]).toContain('LOWER(name) LIKE $1 OR LOWER(description) LIKE $2');
    ...
    it('should validate search columns', async () => {
      await expect(service.search('test', ['invalid_column'])).rejects.toThrow('No valid search columns provided');
    });
