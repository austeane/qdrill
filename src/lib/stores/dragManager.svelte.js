...
export function getDragStateSnapshot() {
	return $state.snapshot(dragState);
}

export function setDragState(next) {
	Object.assign(dragState, { ...INITIAL_DRAG_STATE, ...(next ?? {}) });
}
...
function initializeDrag(event) {
	...
	const priorState = getDragStateSnapshot();
...
export function handleDrop(event) {
	try {
		...
		// Get current drag state
		let state = getDragStateSnapshot();
