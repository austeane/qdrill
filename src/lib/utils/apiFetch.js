...
export async function apiFetch(url, opts = {}) {
  ...
  if (!response.ok) {
    ...
    } else if (typeof body === 'string' && body.length > 0 && body.length < 500) { // Use text body if it's reasonable
      message = body;
  ...
  if (parseError && typeof body !== 'string') {
      throw new Error(`Successfully fetched, but failed to parse response body: ${parseError.message}`);
  }
