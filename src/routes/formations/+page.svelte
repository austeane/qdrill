...
<div class="container mx-auto px-4 py-8">
  ...
  <div class="flex justify-between items-center mb-6">
    <div>
      ...
      <p class="text-gray-600 mt-1">Offensive and defensive formations for half-court situations.</p>
    </div>
    <button
  ...
  <div class="bg-gray-50 rounded-lg p-6 mb-8">
    ...
    {#if filterOptions.tags && filterOptions.tags.length > 0}
      <div class="mt-4" data-testid="filter-category-tags">
        ...
        <div class="flex flex-wrap gap-2" data-testid="tag-select-box"> <!-- Adjusted selector target for test simplicity -->
  ...
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {#each $formations as formation (formation.id)} <!-- Iterate over $formations -->
        <a href="/formations/{formation.id}" data-testid="formation-card" class="block bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
          <div class="p-6">
            <h2 data-testid="formation-card-name" class="text-xl font-semibold text-gray-800 mb-2 truncate">{formation.name}</h2>
            ...
            {#if formation.tags && formation.tags.length > 0}
              <div class="flex flex-wrap gap-1.5 mt-2 mb-4">
                {#each formation.tags.slice(0, 3) as tag} <!-- Limit displayed tags -->
                  <span class="px-2 py-0.5 bg-gray-100 text-gray-700 text-xs rounded-full border border-gray-200">
                    {tag}
                  </span>
                {/each}
            ...
            <!-- Metadata -->
            <div class="flex items-center justify-between mt-auto text-xs text-gray-500 border-t border-gray-100 pt-3">
              <span data-testid="formation-card-type">Type: {formation.formation_type || 'N/A'}</span>
              <span data-testid="formation-card-date">{new Date(formation.created_at).toLocaleDateString()}</span>
