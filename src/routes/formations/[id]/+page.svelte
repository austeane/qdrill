...
<svelte:head>
  <title>{formation?.name || 'Formation'} - QDrill</title> <!-- Use formation directly -->
      ...
      {#if formation.tags && formation.tags.length > 0}
        <div class="flex flex-wrap gap-2 mb-6">
          {#each formation.tags as tag}
            <span class="px-3 py-1 bg-blue-50 text-blue-600 text-sm rounded-full border border-blue-100">
              {tag}
            </span>
          {/each}
        </div>
        ...
        <div class="border-t border-gray-200 pt-6">
          ...
          <div class="space-y-8">
            {#each formation.diagrams as diagramData, i}
              {@const diagram = (typeof diagramData === 'string') ? JSON.parse(diagramData) : diagramData}
              {#if diagram}
        ...
        <div class="flex flex-wrap justify-between gap-x-4 gap-y-1">
          ...
          <div class="flex items-center gap-x-2">
            {#if formation.formation_type}
              <span>Type: {formation.formation_type.charAt(0).toUpperCase() + formation.formation_type.slice(1)}</span>
