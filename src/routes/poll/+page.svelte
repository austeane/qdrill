    ...
    async function vote(optionId) {
        if (voteDebounce.has(optionId)) {
            ...
            if (Date.now() - lastVote < 30000) {
                toast.push('Please wait before voting again', { theme: { '--toastBackground': '#F56565' } });
                return;
            }
        }
    ...
    <div class="bg-bg-0 rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-4">
            ...
            <div class="flex gap-2">
                <button
                    class="px-3 py-1 rounded-md text-sm {sortBy === 'votes' ? 'bg-blue-500 text-white' : 'bg-white text-text hover:bg-gray-50'}"
                    on:click={() => sortOptions('votes')}
                >
                    Most Voted
                </button>
                <button
                    class="px-3 py-1 rounded-md text-sm {sortBy === 'date' ? 'bg-blue-500 text-white' : 'bg-white text-text hover:bg-gray-50'}"
                        ...
                        <div class="flex items-start justify-between">
                            <div class="flex-1">
                                ...
                                {#if option.drill_link}
                                    <a href={option.drill_link} class="text-blue-500 hover:underline text-sm mt-1 inline-block">
                                        View Drill →
                                    </a>
                                            ...
                                            {#if drillOptions.length > 0}
                                                <div class="absolute z-10 w-full mt-1 bg-white border rounded-md shadow-lg max-h-60 overflow-auto">
                                                    {#each drillOptions as drill}
                                                        <button
                                                            class="w-full text-left px-3 py-2 hover:bg-gray-100 {selectedDrill?.id === drill.id ? 'bg-blue-50' : ''}"
                                                            on:click={() => {
                                                                selectedDrill = drill;
