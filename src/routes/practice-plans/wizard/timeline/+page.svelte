<script>
    ...
    $: {
        ...
        if (addedSections.length > 0) {
            needsUpdate = true;
            const defaultDuration = sections.length > 0 ? Math.floor($basicInfo.totalTime / sections.length) : 15;
        ...
        let reorderedAndUpdated = sections.map(section => {
    ...
    $: totalTimeUsed = $timeline.sections.reduce((total, section) => total + (section.duration || 0), 0);
